...
demo_puzzle_data = [110, 95, 210];
demo_puzzle_y_data = [1, 2, 3];

enfant_terrible_data = [102, 85, 189, 92];
enfant_terrible_y_data = [1, 2, 3, 4];
terrible_ave_attempts = [3];
terrible_ave_resets = [3];
terrible_y_attempts = [1];
terrible_y_resets = [1];

noire_data = [134, 80, 145];
noire_ave_attempts = [3];
noire_ave_resets = [3];
noire_y_time = [1, 2, 3];
noire_y_attempts = [1];
noire_y_resets = [1];
...
machina_data = [154, 78, 209];
machina_y_data = [1, 2, 3];
machina_ave_attempts = [3];
machina_ave_resets = [3];
machina_y_attempts = [1];
machina_y_resets = [1];

quarterback_data = [83, 105, 121, 162, 176];
quarterback_y_data = [1, 2, 3, 4, 5];
quarterback_ave_attempts = [1, 2, 3, 4, 5];
quarterback_ave_resets = [1, 2, 3, 4, 5];
quarterback_y_attempts = [1, 2, 3, 4, 5];
quarterback_y_resets = [1, 2, 3, 4, 5];


function d3_charts(left, left_inner, right, right_inner, ave_score, y_axis, user_time) {
    var a = d3.scale.linear().domain([0, d3.max(ave_score)]).range([0, 385]);
    var b = d3.scale.linear().domain([0, d3.max(user_time)]).range([0, 385]);
    var h = d3.scale.ordinal().domain(ave_score).rangeBands([0, 120]);
    var e = d3.scale.ordinal().domain(user_time).rangeBands([0, 120]);
    var g = d3.select(left).append("svg:svg").attr("class", "chart3").attr("width", 400).attr("height", 140).append("svg:g").attr("transform", "translate(10,15)");
    var f = d3.select(left_inner).append("svg:svg").attr("class", "chart3").attr("width", 400).attr("height", 140).append("svg:g").attr("transform", "translate(10,15)");
    var d = d3.select(right).append("svg:svg").attr("class", "chart4").attr("width", 400).attr("height", 140).append("svg:g").attr("transform", "translate(10,15)");
    var c = d3.select(right_inner).append("svg:svg").attr("class", "chart4").attr("width", 400).attr("height", 140).append("svg:g").attr("transform", "translate(10,15)");
